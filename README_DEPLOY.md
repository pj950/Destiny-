...
RAZORPAY_KEY_SECRET=your_test_secret... # or your_live_secret... for production
RAZORPAY_WEBHOOK_SECRET=your_webhook_secret... # from Razorpay webhook endpoint settings
...
| `RAZORPAY_KEY_SECRET` | Yes | Razorpay key secret for payments | `your_test_secret...` or `your_live_secret...` |
...
| `RAZORPAY_KEY_SECRET` | `your_test_secret...` or `your_live_secret...` | Production, Preview, Development | Razorpay key secret for payments (use test secrets for testing) |
| `RAZORPAY_WEBHOOK_SECRET` | `your_webhook_secret...` | Production, Preview, Development | Razorpay webhook signing secret (see Step 5.6 below) |
...
- ⚠️ Leave `RAZORPAY_WEBHOOK_SECRET` blank initially - you'll add it after deployment (Step 5.6)
...
The Razorpay webhook is required for payment confirmation and report generation.
...
4. Configure webhook:
   ...
   - **Secret**: Leave this blank for now - Razorpay will generate it after creation
   - **Active**: Toggle **ON**
   - **Events**: Click **"Select Events** and choose:
     - ✅ Check `payment_link.paid`
     - ✅ Check `payment_link.partially_paid` (optional, for future use)
5. Click **"Create Webhook"**

#### Step 5.6.2: Get Webhook Secret

1. After creating the webhook, click on it to view details
2. Copy the **Webhook Secret** (a long alphanumeric string)
...
4. Select `payment_link.paid`
5. Click **"Send Test"**
6. Verify the webhook was received successfully (check response status in your Vercel function logs)

#### Note: Stripe Webhook Retirement

The old Stripe webhook endpoint (`/api/stripe/webhook`) has been retired and now returns a 410 Gone response. Any existing Stripe webhooks should be removed from your Stripe dashboard to avoid unnecessary failed requests.
...
1. Go to [razorpay.com](https://razorpay.com) and sign up/sign in
...
4. Copy both the **Key ID** and **Key Secret** for your mode
5. Add them to your environment variables:
   - `RAZORPAY_KEY_ID` (the Key ID)
   - `RAZORPAY_KEY_SECRET` (the Key Secret)

### Step 7.2: Razorpay Payment Links

The application uses Razorpay Payment Links for both lamp purchases and report generation. Unlike traditional payment methods, payment links:

- **Don't require predefined products**: Prices are set dynamically in the API
- **Support multiple currencies**: USD is used for international transactions
- **Include built-in checkout**: Users complete payment on Razorpay's hosted page
- **Provide webhook callbacks**: Automatic notifications when payments are completed

### Step 7.3: Razorpay Webhooks

The application uses Razorpay webhooks to finalize payment flows after successful payment.

#### Development Testing with ngrok

For local development, use ngrok to expose your local server to the internet:

1. **Install ngrok**: Follow instructions at [ngrok.com/download](https://ngrok.com/download)
...
6. **Test with a real checkout**:
   - Create a chart and initiate report generation or lamp purchase
   - Complete Razorpay payment (use test credentials provided by Razorpay)
   - Verify webhook is received and processed correctly
   - Check that jobs/lamps are updated in database
...
4. Select events to listen for:
   - `payment_link.paid`
   - `payment_link.partially_paid` (optional)
...
1. User completes Razorpay payment via payment link
2. Razorpay sends `payment_link.paid` event to your webhook endpoint
3. Webhook validates the signature using `RAZORPAY_WEBHOOK_SECRET`
4. Webhook extracts identifiers from payment link:
   - For lamps: `lamp_key` and `purchase_type` from notes
   - For reports: `chart_id` and `purchase_type` from notes
5. **For lamp purchases**: Updates lamp status to `lit` and stores `razorpay_payment_id`
6. **For report purchases**: Finds job by `razorpay_payment_link_id` and updates to `status='pending'`
7. Webhook implements idempotency by storing `last_razorpay_webhook_event_id` to prevent duplicate processing
8. Background worker picks up pending jobs and generates reports

#### Event Mapping

| Razorpay Event | Action | Database Update |
|---------------|--------|-----------------|
| `payment_link.paid` | Payment successful | Update lamp to `status='lit'` OR update job to `status='pending'` with `payment_confirmed: true` |
| `payment_link.partially_paid` | Partial payment (future) | Log event, no status change (for future features) |

#### Security Features

- **Signature Verification**: All webhook events are verified using Razorpay's HMAC SHA256 signature
- **Idempotency**: Duplicate events are detected and ignored using `last_razorpay_webhook_event_id`
...
- **Razorpay**: Check **Settings** > **Webhooks** for payment events
...
- [ ] Google API key is valid and has credits
